"""
This project was created at 1403 2016
The purpose of this project is to write a Chinese RPYS for social scientist in China.
Created by Xin Li.
add an interface for the data download from web of science. 20180913
"""
import re
import os
import time
...
if os.path.isfile("cssci.txt"):
    ...
    with open("cssci.txt", encoding="utf8") as dataFile:
        while 1:
    ...
    with open("rpys_cssci.csv", "a", encoding="utf8") as resultFile:
        for rs in Result:
            resultFile.write(rs)
    ...
    for yy in range(2, len(times[:-3])):
        ss = int(times[count - 2]) + int(times[count - 1]) + int(times[count]) + int(times[count + 1]) + int(
            times[count + 2])
        avg = ss / 5
    ...
    with open("median_cssci.csv", "a", encoding="utf8") as MedianResultFile:
...
if os.path.isfile("wos.txt"):
    ...
    with open("rpys_wos.csv", "a", encoding="utf8") as resultFile:
        for rs in Result:
            resultFile.write(rs)
    ...
    with open("median_wos.csv", "a", encoding="utf8") as MedianResultFile:
        for mrs in MedianResult:
            MedianResultFile.write(mrs + "\n")
else:
    print("-------------------------未检测到wos.txt-------------------------" + '\n\n')
